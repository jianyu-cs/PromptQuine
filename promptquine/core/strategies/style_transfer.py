"""Style transfer task strategy implementation."""

from .base import TaskStrategy
from promptquine.utils.style_transfer import (
    StyleTransferPromptCandidate,
    get_style_transfer_field,
    aggregate_style_transfer_results,
    should_be_evaluated_next_round_for_style_transfer
)
from promptquine.examples.style_transfer.tst_evaluator import (
    PromptedStyleTransferEvaluator
)


class StyleTransferStrategy(TaskStrategy):
    """Strategy for style transfer tasks."""
    
    @classmethod
    def create_candidate(cls, prompt, mask, eval_result, tokenizer):
        return StyleTransferPromptCandidate.from_evaluation(
            prompt, mask, eval_result, tokenizer
        )
    
    @classmethod
    def should_evaluate_next_round(cls, eval_result, min_reward, reward_driven=None):
        return should_be_evaluated_next_round_for_style_transfer(
            eval_result, min_reward
        )
    
    @classmethod
    def get_field(cls, lst, key, default=None, reward_driven=None):
        return get_style_transfer_field(lst, key, default, reward_driven)
    
    @classmethod
    def create_evaluator(cls, **kwargs):
        return PromptedStyleTransferEvaluator(**kwargs)
    
    @classmethod
    def aggregate_results(cls, results):
        return aggregate_style_transfer_results(results)

